...
При кратком сбое резолвер может отдавать недавно устаревшие ответы из кэша. `vpnproxi-dns-health.timer` каждые 15 секунд проверяет уникальное имя мимо кэша и отслеживает переполнение очереди dnsmasq. После двух последовательных нездоровых окон перезапускается dnsmasq; если сбой сохраняется, Xray может быть перезапущен не чаще одного раза в пять минут.
