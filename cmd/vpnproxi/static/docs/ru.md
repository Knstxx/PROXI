...
При кратком сбое резолвер может отдавать недавно устаревшие ответы из кэша. `vpnproxi-dns-health.timer` каждые 15 секунд проверяет уникальное имя мимо кэша. После двух последовательных ошибок перезапускается dnsmasq; если сбой сохраняется, Xray может быть перезапущен не чаще одного раза в пять минут.
