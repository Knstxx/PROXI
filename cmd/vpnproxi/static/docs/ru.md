...
При кратком сбое резолвер может отдавать недавно устаревшие ответы из кэша. `vpnproxi-dns-health.timer` каждые 15 секунд проверяет уникальное имя мимо кэша. Успешная проверка всегда считается здоровым состоянием, даже если в журнале остались старые сообщения о переполнении очереди. После двух последовательных неудачных проверок перезапускается dnsmasq, между перезапусками резолвера выдерживается две минуты, а Xray может быть перезапущен не чаще одного раза в пять минут.
