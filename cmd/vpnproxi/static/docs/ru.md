...
Direct-правила имеют приоритет выше proxy-правил. DNS к шлюзу и частные домашние/служебные подсети обходят прозрачную классификацию трафика. Основной upstream локального кэша dnsmasq — параллельный DNS-over-HTTPS Xray через настроенный внешний outbound; это не меняет маршрут последующего соединения с сайтом. Для аварии хранятся отдельные шаблоны Cloudflare и Google; health-check включает только один проверенный прямой резолвер и только на время недоступности защищённого моста.
...
Файлы устанавливаются в `/usr/local/share/xray`. После успешного обновления Xray перезапускается, чтобы загрузить новые категории. `vpnproxi-dnsmasq` остается только небольшим DNS-кэшем и не участвует в выборе маршрута. Обратные запросы для частных адресов и имён `.local` завершаются локально и не занимают очередь внешнего резолвера. В режимах Selective и Force upstream-запросы A/AAAA идут через параллельный DNS-over-HTTPS в Xray вместо обычного UDP/TCP-порта 53; лимиты ожидающих DNS-запросов и TCP-соединений также ограничены.

При кратком сбое резолвер может отдавать недавно устаревшие ответы из кэша. `vpnproxi-dns-health.timer` каждые пять секунд отдельно проверяет клиентский путь dnsmasq и основной DNS-мост Xray. После двух сбоев primary он атомарно переключает `servers-file` dnsmasq на прямой резерв через `SIGHUP`; возврат требует шести успешных проверок и минимум двух минут работы на fallback. После двух последовательных сбоев клиентского пути перезапускается только dnsmasq с двухминутным cooldown. DNS-мониторинг никогда не перезапускает общий Xray и поэтому не рвёт активные проксированные звонки.
